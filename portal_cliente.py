            ...
            try:
                ...
                if len(dados_app) > 1:
                    ...
                    def extrair_dados_app(r):
                        ...
                        s = s if s.upper() != 'NAN' else ''; o = o if o.upper() != 'NAN' else ''
                        d = d if d.upper() != 'NAN' else ''; rec = rec if rec.upper() != 'NAN' else ''
                        f = f if f.upper() != 'NAN' else ''; q = d if d else rec
    ...
    if not df_raw.empty:
        ...
        if not df_cliente.empty:
            ...
            def tratar_status(row):
                ...
                if res not in ['✅ Entregue', '❌ Frustrada'] and previsao:
                    try:
            ...
            with st.sidebar:
                ...
                with st.popover("⚙️ Personalizar Colunas", use_container_width=True):
                    col_vis = st.multiselect("Ver:", options=col_disponiveis, default=col_disponiveis)
                st.divider()
            ...
            # 🛠️ FILTRO FINAL E BUSCA PARA EXPORTAÇÃO
            ...
            with st.sidebar:
                ...
                st.divider(); st.button("🚪 Sair do Sistema", use_container_width=True, on_click=lambda: st.session_state.update({"logado": False}))
            ...
            if not df_final_view.empty:
                ...
                df_final_grid = df_final_view[[c for c in col_vis if c in df_final_view.columns]]
