    ...
    if not df_raw.empty:
        ...
        if not df_cliente.empty:
            ...
            def processar_detalhes(row):
                s = str(row.get('APP_STATUS', row.get('STATUS', ''))).upper()
                if 'FRUSTRADA' in s:
            ...
            def tratar_status(row):
                s, previsao = str(row.get('APP_STATUS', row.get('STATUS', ''))).strip().upper(), str(row.get('DATA_LIMITE', '')).strip()
            ...
            with st.sidebar:
                ...
                st.divider()
                # BOTÕES DE WHATSAPP E EXCEL NA SIDEBAR
            ...
            # SIDEBAR - DISPARO WHATSAPP E EXCEL DO FILTRADO
            with st.sidebar:
            ...
            if not df_grid.empty:
                ...
                cols = ['DATA', 'PEDIDO', 'STATUS', 'LABORATORIO', 'CIDADE', 'UF', 'BAIRRO', 'DATA_LIMITE', 'FOTO_URL', 'DETALHES']
                ...
                status_js = JsCode("""function(params) { let v = params.value || ''; if (v.includes('Entregue')) return {'backgroundColor': 'rgba(16, 185, 129, 0.15)', 'color': '#10B981', 'fontWeight': '900'}; if (v.includes('Frustrada') || v.includes('ATRASADO')) return {'backgroundColor': 'rgba(239, 68, 68, 0.15)', 'color': '#EF4444', 'fontWeight': '900'}; return {'fontWeight': 'bold'}; }""")
