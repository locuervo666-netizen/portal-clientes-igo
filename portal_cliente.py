...
# =======================================================
# 🎨 1. CONFIGURAÇÃO DA PÁGINA E CSS BASE (RESTAURADO INTEGRAL)
# =======================================================
...
# =======================================================
# 🔗 MOTOR DE DADOS PRINCIPAL (ESTRUTURA ORIGINAL COMPLETA)
# =======================================================
@st.cache_resource
    ...
    if not df_raw.empty:
        ...
        if not df_cliente.empty:
            ...
            with st.sidebar:
                ...
                n_frus = len(df_cliente[df_cliente['STATUS_DISPLAY'].str.contains('Frustrada')])
                n_atra = len(df_cliente[df_cliente['STATUS_DISPLAY'].str.contains('ATRASADO')])
                texto_w = f"*Resumo IGO - {st.session_state.cliente}*\n📦 Total: {n_tot}\n✅ OK: {n_ent}"
                st.markdown(f'<a href="https://api.whatsapp.com/send?text={urllib.parse.quote(texto_w)}" target="_blank"><button style="width:100%; background:#25D366; color:white; border:none; padding:10px; border-radius:8px; font-weight:bold; cursor:pointer;">📲 Disparar WhatsApp</button></a>', unsafe_allow_html=True)
            ...
            st.markdown(f"""<style> [data-testid="stAppViewContainer"] {{ background-color: {bg_app} !important; }} .dinamic-text {{ color: {txt_main} !important; }} </style>""", unsafe_allow_html=True)
            ...
            busca_inteligente = st.text_input("🔎 Busca Rápida:", placeholder="Ex: Melo Labs, Centro, Maria, Frustrada, 102938...")
            df_grid = df_f.copy()
            ...
            if busca_inteligente:
                mask = df_grid.astype(str).apply(lambda x: x.str.lower().str.contains(busca_inteligente.lower())).any(axis=1)
                ...
                class FotoRenderer {
                    init(params) {
                        ...
                        if (params.value && params.value.includes('http')) {
                            let b = document.createElement('span'); b.innerHTML = '📸'; b.style.cursor='pointer';
                            b.onclick = () => {
                                let m = document.createElement('div');
                                Object.assign(m.style, {position:'fixed', zIndex:'9999', left:0, top:0, width:'100%', height:'100%', background:'rgba(0,0,0,0.9)', display:'flex', justifyContent:'center', alignItems:'center'});
