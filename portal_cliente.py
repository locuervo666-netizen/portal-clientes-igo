...
# =======================================================
# 🎨 1. CONFIGURAÇÃO E CSS (ESTABILIZADO)
# =======================================================
...
st.markdown("""
    ...
    .bg-total { background: linear-gradient(135deg, #1E3A8A 0%, #3B82F6 100%); }
    .bg-frus { background: linear-gradient(135deg, #9A3412 0%, #F59E0B 100%); }
    .bg-atra { background: linear-gradient(135deg, #7F1D1D 0%, #EF4444 100%); }
    .bg-hoje { background: linear-gradient(135deg, #064E3B 0%, #10B981 100%); }
    
    .val { font-size: 32px; font-weight: 900; line-height: 1; }
    .lab { font-size: 11px; font-weight: 700; text-transform: uppercase; opacity: 0.8; }
    
    /* Ajuste nos botões de filtro */
    .stButton>button { width: 100%; border-radius: 5px; font-weight: bold; font-size: 12px; height: 35px; }
    
    h1 { color: #0f172a; font-weight: 900; font-size: 26px; margin-bottom: 10px; }
...
if not st.session_state.logado:
    ...
    if not df_raw.empty:
        # Filtro por cliente
        if st.session_state.cliente == "IGO_LOGISTICA": df_cliente = df_raw.copy()
        else: df_cliente = df_raw[df_raw['TOMADOR'] == st.session_state.cliente].copy()
        ...
        # Tratamento de Status e Detalhes
        ...
        # Aplicação de Filtros da Sidebar
        ...
        # --- RENDERIZAÇÃO DOS CARDS (GRID 4 COLUNAS) ---
        c1, c2, c3, c4 = st.columns(4)
        
        with c1:
            st.markdown(f'<div class="kpi-box bg-total"><div class="lab">Total Filtrado</div><div class="val">{n_total}</div></div>', unsafe_allow_html=True)
            if st.button("Ver Todos", key="f_todos"): st.session_state.filtro_kpi = "TODOS"
        with c2:
            st.markdown(f'<div class="kpi-box bg-frus"><div class="lab">Frustradas</div><div class="val">{n_frus}</div></div>', unsafe_allow_html=True)
            if st.button("Filtrar Frustradas", key="f_frus"): st.session_state.filtro_kpi = "FRUSTRADA"
        with c3:
            st.markdown(f'<div class="kpi-box bg-atra"><div class="lab">Atrasadas</div><div class="val">{n_atra}</div></div>', unsafe_allow_html=True)
            if st.button("Filtrar Atrasadas", key="f_atra"): st.session_state.filtro_kpi = "ATRASADO"
        with c4:
            st.markdown(f'<div class="kpi-box bg-hoje"><div class="lab">Para Hoje</div><div class="val">{n_hoje}</div></div>', unsafe_allow_html=True)
            if st.button("Filtrar Hoje", key="f_hoje"): st.session_state.filtro_kpi = "HOJE"

        # Lógica de Filtro do Clique
        df_grid = df_f.copy()
        if st.session_state.filtro_kpi == "FRUSTRADA":
            df_grid = df_grid[df_grid['STATUS_F'].str.contains('Frustrada', na=False)]
            st.info("🎯 Mostrando apenas **Coletas Frustradas**")
        elif st.session_state.filtro_kpi == "ATRASADO":
            df_grid = df_grid[df_grid['STATUS_F'].str.contains('ATRASADO', na=False)]
            st.warning("🚨 Mostrando apenas **Pedidos Atrasados**")
        elif st.session_state.filtro_kpi == "HOJE":
            df_grid = df_grid[df_grid['DATA_OBJ'] == hoje_br]
            st.success("📅 Mostrando apenas **Pedidos de Hoje**")
