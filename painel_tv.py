...
# =============================================================================
# 2. CONEXÃO COM O BANCO DE DADOS (CÓPIA BLINDADA)
# =============================================================================
@st.cache_resource
def conectar_banco():
...
@st.cache_data(ttl=20) # Cache curto para TV manter atualização rápida
...
# =============================================================================
# 🎨 3. CSS NASDAQ PREMIUM E FUNÇÕES DE TELA
# =============================================================================
st.markdown("""
<style>
    [data-testid="stAppViewContainer"] { background-color: #020617 !important; }
    [data-testid="stHeader"] { background-color: transparent !important; }
    .tv-header { display: flex; justify-content: space-between; align-items: center; padding: 10px 30px; border-bottom: 2px solid #1E293B; margin-bottom: 30px; }
    .tv-clock { color: #38BDF8; font-family: 'Courier New', Courier, monospace; font-size: 32px; font-weight: bold; }
    h3 { color: #F8FAFC !important; font-weight: 800 !important; }
...
def criar_card(titulo, valor, subtitulo, cor_acento):
    return f"""
    <div style="background-color: #0B1120; border-left: 5px solid {cor_acento}; padding: 25px; border-radius: 10px; box-shadow: 0 10px 15px -3px rgba(0,0,0,0.5); height: 160px; display: flex; flex-direction: column; justify-content: center; margin-bottom: 20px; border-right: 1px solid #1E293B; border-top: 1px solid #1E293B; border-bottom: 1px solid #1E293B;">
        <p style="margin:0; font-size: 15px; font-weight: 800; color: #94A3B8; text-transform: uppercase; letter-spacing: 1px;">{titulo}</p>
        <h1 style="margin:0; font-size: 56px; font-weight: 900; color: #F8FAFC; font-family: 'Courier New', monospace; letter-spacing: -2px;">{valor}</h1>
        <p style="margin:0; font-size: 14px; font-weight: 600; color: {cor_acento}; margin-top: 5px;">{subtitulo}</p>
    </div>
    """
...
st.markdown(f"""
<div class="tv-header">
    <img src="https://i.postimg.cc/x84nnjjq/IGO-LOGO.png" width="200">
...
if df_raw.empty:
    ...
    # ================== SLIDE 1: VISÃO GERAL (NASDAQ) ==================
    if st.session_state.slide_atual == 1:
        st.markdown("<h3 style='color: #38BDF8 !important; padding-left: 10px;'>📊 VISÃO MACRO - TRACKING DO DIA</h3>", unsafe_allow_html=True)
        ...
        with c8:
            ...
            st.markdown("<h4 style='color: #F8FAFC;'>🎯 Eficiência Tática</h4>", unsafe_allow_html=True)
            ...
            st.markdown(f"<p style='font-size:14px; color:#94A3B8; margin-top:-5px; font-weight:600;'>Ação registrada em <b>{concluidos_globais}</b> de <b>{total_hoje}</b> alvos.</p>", unsafe_allow_html=True)

    # ================== SLIDE 2: DESEMPENHO DOS MOTORISTAS ==================
    elif st.session_state.slide_atual == 2:
        st.markdown("<h3 style='color: #38BDF8 !important; padding-left: 10px;'>🏆 PERFORMANCE INDIVIDUAL EM SOLO</h3>", unsafe_allow_html=True)
        ...
        if not df_hoje.empty:
            ...
            # CSS Dark para tabela da TV
            st.markdown("""
            <style>
            [data-testid="stDataFrame"] { background-color: #0B1120; padding: 20px; border-radius: 12px; border: 1px solid #1E293B; }
            th { color: #38BDF8 !important; font-size: 16px !important; }
            td { color: #F8FAFC !important; font-size: 18px !important; font-weight: bold; }
            </style>
            """, unsafe_allow_html=True)
            ...
            st.dataframe(
                ...
                column_config={
                    ...
                    "% Concluído": st.column_config.ProgressColumn("📊 Barra de Combate", format="%f%%", min_value=0, max_value=100),
                },
    ...
    # ================== SLIDE 3: DISTRIBUIÇÃO E GARGALOS ==================
        ...
        st.markdown("<h3 style='color: #38BDF8 !important; padding-left: 10px;'>📊 RAIO-X DE DISTRIBUIÇÃO E TEMPERATURA</h3>", unsafe_allow_html=True)
        ...
        with col1:
            if not df_hoje.empty:
                ...
                st.bar_chart(data=status_counts, x='Status', y='Quantidade', color='#0284C7', height=500)
        
        with col2:
            st.markdown(criar_card("VOLUME RESTANTE", pendentes, "Falta Coletar", "#64748B"), unsafe_allow_html=True)
            st.markdown(criar_card("TAXA DE FALHA", f"{int((frustrados/total_hoje)*100) if total_hoje > 0 else 0}%", "Proporção de problemas", "#F43F5E"), unsafe_allow_html=True)

# Rodapé de paginação
st.markdown("<br><br>", unsafe_allow_html=True)
st.markdown(f"<p style='text-align:center; color:#475569; font-weight:bold;'>Tela {st.session_state.slide_atual} de 3 | Autopilot Ativado</p>", unsafe_allow_html=True)
