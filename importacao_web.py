...
# CSS AGGRID (Vacinado contra JS Modal)
def obter_css_grid():
    return {
...
if menu == "📊 Dashboard":
    ...
    if not df_raw.empty:
        ...
        colunas_mostrar = ['DATA', 'PEDIDO', 'TOMADOR', 'LABORATORIO', 'CIDADE', 'STATUS_DISPLAY', 'FOTO_INDICADOR', 'DATA_LIMITE', 'AGENTE_RAW', 'DATA_ENTREGA']
        ...
        st.markdown(f"<p style='color:#059669; font-weight:600; font-size:12px; margin-bottom: 5px;'>🟢 Sincronizado: {datetime.now(FUSO_BR).strftime('%H:%M:%S')} | Marque a caixinha na tabela para abrir a Foto e os Comandos logo abaixo!</p>", unsafe_allow_html=True)
        
        # 🔥 LAYOUT VERTICAL: Botões no Topo -> Tabela 100% -> Foto em Baixo
        box_botoes = st.empty()
        box_grid = st.container()
        box_foto = st.container()

        # RENDERIZANDO A TABELA E PEGANDO A SELEÇÃO
        with box_grid:
            ...
            gb.configure_column("DATA_ENTREGA", headerName="Data Real Entrega", width=120) 
        ...
        # 🔥 A FOTO APARECE EMBAIXO QUANDO SELECIONADO
        with box_foto:
            if tem_sel:
                st.markdown("---")
                st.markdown(f"<h3 style='margin-top:0; color:#0369A1;'>📸 Comprovantes ({len(p_ids)} pedidos)</h3>", unsafe_allow_html=True)
                ...
                if not fotos_validas:
                    st.info("Nenhuma foto encontrada para os pedidos selecionados.")
                else:
                ...
                if not tem_sel: st.warning("Selecione um pedido na tabela abaixo primeiro!")
                    ...
                    if st.button("Confirmar Clone", type="primary", use_container_width=True):
                        with st.spinner("Clonando na base..."):
                            ...
                            except Exception as e: st.error(f"Erro no Clone: {e}")
            ...
            if st.form_submit_button("🚀 Injetar na Base e Roteirizar", type="primary", use_container_width=True):
                if m_tomador == "Selecione..." or not m_cid or not m_lab or not m_rua or not m_bai: 
                    ...
                    with st.spinner("Padronizando textos e salvando na nuvem..."):
                        ...
                        m_prazo = str(calcular_sla_dias("SP", cid_limpa))
                        ...
                        try:
                            ...
                            novo_ped = pd.DataFrame([{'DATA': m_data.strftime("%d/%m/%Y"), 'PEDIDO': m_pedido, 'TOMADOR': m_tomador, 'LABORATORIO': lab_limpo, 'ENDERECO': rua_limpa, 'NUMERO': "", 'BAIRRO': bai_limpo, 'CIDADE': cid_limpa, 'UF': "SP", 'CEP': "", 'STATUS': 'PENDENTE', 'AGENTE_RAW': m_agente, 'PRAZO_DIAS': m_prazo, 'DATA_LIMITE': m_limite, 'DATA_ENTREGA': "", 'FOTO': "", 'ROMANEIO': ""}]).astype(str)
                ...
                try:
                    ...
                    for idx, row in df_limpo.iterrows():
                        ...
                        if e and (not n or not b):
                            ...
                            if ',' in e and not n: pts = e.split(','); df_limpo.at[idx, 'ENDERECO'] = pts[0].strip(); df_limpo.at[idx, 'NUMERO'] = pts[1].strip()
...
# =============================================================================
# 📋 MÓDULO 3: TRIAGEM E ROMANEIO (PDF E TABELA MANUAL RESTAURADOS)
# =============================================================================
elif menu == "🔬 Triagem":
    ...
    if not df_raw.empty:
        ...
        with t1:
            ...
            if not df_fila.empty:
                ...
                gb_fila.configure_default_column(resizable=True, sortable=True, filter=False, flex=1)
                gb_fila.configure_selection('multiple', use_checkbox=True, header_checkbox=True)
                grid_fila_resp = AgGrid(df_fila, gridOptions=gb_fila.build(), theme='alpine', custom_css=obter_css_grid(), height=300, fit_columns_on_grid_load=False, update_mode=GridUpdateMode.SELECTION_CHANGED)
        ...
        with t2:
            ...
            if not df_conf.empty:
                ...
                gb_conf.configure_default_column(resizable=True, sortable=True, filter=False, flex=1)
                gb_conf.configure_selection('multiple', use_checkbox=True, header_checkbox=True)
                grid_conf_resp = AgGrid(df_conf_show, gridOptions=gb_conf.build(), theme='alpine', custom_css=obter_css_grid(), height=300, fit_columns_on_grid_load=False, update_mode=GridUpdateMode.SELECTION_CHANGED)
                
                selecionados = grid_conf_resp['selected_rows']
                tem_sel_pdf = False
                if selecionados is not None:
        ...
        with t3:
            ...
            if not df_hist.empty:
                ...
                df_hist_show = df_hist[['DATA', 'PEDIDO', 'TOMADOR', 'LABORATORIO', 'CIDADE', 'STATUS', 'AGENTE_RAW', 'ROMANEIO']]
                
                gb_hist = GridOptionsBuilder.from_dataframe(df_hist_show)
                gb_hist.configure_default_column(resizable=True, sortable=True, filter=True, flex=1)
                st_js = JsCode("""
                function(p){
                    let v = p.value ? String(p.value).toUpperCase() : ''; 
                    if(v.includes('ENTREGUE')){ return {'backgroundColor':'rgba(16,185,129,0.1)','color':'#059669','fontWeight':'700'}; } 
                ...
                """)
                gb_hist.configure_column("STATUS", headerName="STATUS", cellStyle=st_js)
                ...
                with st.expander(f"👤 Agente Tático: {str(agente).upper()} | Volumes: {len(df_agente)}", expanded=False):
                    ...
                    if telefone:
                        ...
                        for i, (_, row) in enumerate(df_agente.iterrows(), 1):
                            ...
                            if str(row.get('OBSERVACOES', '')).strip() and str(row.get('OBSERVACOES', '')).upper() != 'NAN': msg += f"📝 *Aviso:* {row['OBSERVACOES']}\n"
                            msg += "------------------------\n"
