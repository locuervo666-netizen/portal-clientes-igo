...
# =============================================================================
# 🚀 MÓDULO 1: DASHBOARD (LAYOUT FIXO ACIMA + AGGRID SEGURA)
# =============================================================================
if menu == "📊 Dashboard":
    ...
    if not df_raw.empty:
        ...
        # Cria as colunas para o Campo de Busca e para os Botões Fixos em Cima!
        col_busca, col_vazia = st.columns([1, 1])
        busca = col_busca.text_input("🔎 Busca Rápida de Rastreio (Código, Lab, Cidade...):", placeholder="Filtrar dados na tabela...")
        ...
        # Coluna de Texto Segura para Foto (Evita o Javascript Crashing)
        df_grid['FOTO_STATUS'] = df_grid['FOTO_URL'].apply(lambda x: '📸 Sim' if x else '❌ Não')

        colunas_mostrar = ['DATA', 'PEDIDO', 'QR_CODE', 'TOMADOR', 'LABORATORIO', 'CIDADE', 'UF', 'STATUS_DISPLAY', 'FOTO_STATUS', 'DATA_LIMITE', 'AGENTE_RAW', 'DATA_ENTREGA']
        ...
        st.markdown(f"<p style='color:#059669; font-weight:600; font-size:12px; margin-bottom: 10px;'>🟢 Sincronizado: {datetime.now(FUSO_BR).strftime('%H:%M:%S')}</p>", unsafe_allow_html=True)

        # =============================================================================
        # 🔥 ÁREA DOS BOTÕES FIXOS EM CIMA DA TABELA
        # =============================================================================
        st.markdown("""
            <style>
            div[data-testid="stPopover"] > button, button[kind="secondary"] {
                white-space: nowrap !important; overflow: hidden !important; font-weight: 600 !important; font-size: 12px !important; border-radius: 6px !important; height: 35px !important; min-height: 35px !important; padding: 0px 12px !important; border: 1px solid #CBD5E1 !important; background-color: #FFFFFF !important; color: #475569 !important; transition: all 0.2s ease !important; box-shadow: 0 1px 2px rgba(0,0,0,0.05) !important; margin-bottom: 10px;
            }
        ...
        # Configuração de estado para capturar seleções e usá-las nos botões acima da grid
        if 'selected_rows' not in st.session_state:
            st.session_state.selected_rows = []
            
        p_ids = [str(r['PEDIDO']) for r in st.session_state.selected_rows] if st.session_state.selected_rows else []
        tem_sel = len(p_ids) > 0
        ...
        with col_b1.popover("📲 Dar Baixa Manual", use_container_width=True):
            if not tem_sel: st.warning("Selecione um pedido na tabela abaixo primeiro!")
                ...
                senha_reversao = ""
                if tem_entregue:
                    st.warning("⚠️ Atenção: Desfazendo pedido já **ENTREGUE**.")
                    senha_reversao = st.text_input("🔑 Senha (Obrigatória):", type="password")
                        ...
                        try:
                            ...
                            for pid in p_ids:
                                if pid in df_nuvem['PEDIDO'].values:
                                    ...
                                    l_orig['PEDIDO'] = novo_id; l_orig['DATA'] = clone_data.strftime("%d/%m/%Y"); l_orig['STATUS'] = "PENDENTE"; l_orig['DATA_ENTREGA'] = ""; l_orig['FOTO'] = ""; l_orig['ROMANEIO'] = ""
        ...
        with col_b3.popover("🔄 Trocar Motorista", use_container_width=True):
            if not tem_sel: st.warning("Selecione na tabela primeiro!")
                ...
                if tem_entregue: st.error("⚠️ Pedidos ENTREGUES não podem mudar de agente.")
                else:
                    logins_disp = sorted(DF_AGENTES['LOGIN DO AGENTE'].unique().tolist()) if not DF_AGENTES.empty else []
        ...
        with col_b4.popover("🗑️ Excluir Pedido", use_container_width=True):
            if not tem_sel: st.warning("Selecione na tabela primeiro!")
            else:
                st.error(f"⚠️ Apagar **{len(p_ids)}** pedido(s) permanentemente.")
        ...
        gb.configure_column("FOTO_STATUS", headerName="Comprovante", width=110)
        ...
        grid_response = AgGrid(df_grid, gridOptions=gb.build(), allow_unsafe_jscode=True, theme='alpine', custom_css=obter_css_grid(), height=400, fit_columns_on_grid_load=False, update_mode=GridUpdateMode.SELECTION_CHANGED)
        
        # Salva a seleção no estado para atualizar os botões em cima na próxima rodada (ação comum no Streamlit)
        if grid_response['selected_rows'] != st.session_state.selected_rows:
            st.session_state.selected_rows = grid_response['selected_rows']
            st.rerun()

        # =============================================================================
        # 🔥 VISUALIZADOR DE FOTO (SÓ APARECE QUANDO SELECIONA UMA LINHA)
        # =============================================================================
        if tem_sel:
            st.markdown("---")
            st.markdown(f"<h4 style='color:#0369A1;'>📸 Comprovante(s) do(s) Pedido(s): {', '.join(p_ids)}</h4>", unsafe_allow_html=True)
            ...
            if not fotos_validas:
                st.info("Nenhuma foto anexada pelo Agente para esta seleção.")
            else:
                cols_fotos = st.columns(len(fotos_validas) if len(fotos_validas) <= 3 else 3)
                for i, url in enumerate(fotos_validas):
                    with cols_fotos[i % 3]:
                        st.image(url, use_container_width=True, caption=f"Foto Anexada")
            ...
            if st.button("🚀 3. INJETAR LOTE", type="primary"):
                with st.spinner("Injetando..."):
                    try:
                        ...
                        df_ok['STATUS'], df_ok['DATA_ENTREGA'], df_ok['FOTO'], df_ok['ROMANEIO'] = 'PENDENTE', '', '', ''
                        ...
                        if str(df_raw.at[idx, 'STATUS']).strip().upper() == 'COLETADO':
                            try:
                                ...
                                if mask_nuvem.any():
                                    ...
                                    aba.update("A1", [df_nuvem.columns.tolist()] + df_nuvem.fillna("").astype(str).values.tolist())
                                    st.success(f"✅ Pedido {df_raw.at[idx, 'PEDIDO']} VALIDADO!")
                                    carregar_dados_completos.clear()
                            except Exception as e: st.error(f"Erro: {e}")
                ...
                if c_btn.button("🚚 Despachar", type="primary") and not selecionados.empty and motorista_escolhido != "Selecione...":
                    with st.spinner("Gerando Romaneio..."):
                        ...
                        try:
                            ...
                            st.success(f"🎉 Lote {id_romaneio} gerado!"); carregar_dados_completos.clear(); st.rerun()
                        except Exception as e: st.error(f"Erro: {e}")

# =============================================================================
# 📱 MÓDULO EXTRA: DISPARO WHATSAPP E RELATÓRIOS (Ocultados para focar na Grid)
# =============================================================================
elif menu in ["📱 Zap", "📁 Relatórios", "⚙️ Rotas"]:
    st.info("Para que você possa focar 100% no teste do Dashboard e das Baixas, eu abreviei esta parte. Volte para a aba **Dashboard** e veja a mágica acontecer!")
