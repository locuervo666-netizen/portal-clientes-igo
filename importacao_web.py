...
# =============================================================================
st.set_page_config(page_title="CONTROLE OPERACIONAL", layout="wide", page_icon="🚚", initial_sidebar_state="expanded")
    ...
    if not df_raw.empty:
        ...
        def get_detalhes(row):
            ...
            if status_atual in ['FRUSTRADA', 'PROBLEMA', 'CANCELADO']:
                ...
                if obs_limpa: return f"⚠️ {obs_limpa}"
                else: return "⚠️ Sem justificativa"
        ...
        gb.configure_selection('multiple', use_checkbox=True, header_checkbox=True, header_checkbox_filtered_only=True, suppressRowClickSelection=False)
                    ...
                    if linhas_pendentes.empty: st.error("⚠️ Apenas pedidos Não-Entregues podem ser enviados.")
                        ...
                        with st.form("form_zap_grid"):
                            ...
                            if st.form_submit_button("🚀 Disparar para Motorista(s)", type="primary", use_container_width=True):
                                ...
                                df_raw_pendentes = df_raw[df_raw['PEDIDO'].astype(str).str.strip().isin([p.strip() for p in p_ids_pendentes])]
                                ...
                                if len(agentes_validos) > 0:
                                    progress_bar = st.progress(0)
                                    status_text = st.empty()
                                    
                                    for idx_ag, ag in enumerate(agentes_validos):
                    ...
                    with st.spinner("Injetando pedido no sistema..."):
                        ...
                        try:
                            ...
                            novo_ped_dict = {
                                ...
                                'ENDERECO': rua_limpa, 'NUMERO': num_limpo,
...
# =============================================================================
    ...
    st.markdown("<div class='dinamic-border'><h3 class='dinamic-text' style='margin:0;'>➕ Central de Importação de Lotes (Oficial) & Pedidos Fixos</h3></div>", unsafe_allow_html=True)
    ...
    if "contador_fixos_oficial" not in st.session_state: st.session_state.contador_fixos_oficial = None
    
    # 🔥 INICIALIZAR SESSION STATE PARA PEDIDOS FIXOS 🔥
    if 'f_rua_of' not in st.session_state: st.session_state['f_rua_of'] = ""
    if 'f_bai_of' not in st.session_state: st.session_state['f_bai_of'] = ""
    if 'f_cid_of' not in st.session_state: st.session_state['f_cid_of'] = ""
    if 'f_uf_of' not in st.session_state: st.session_state['f_uf_of'] = ""
    if 'cep_version_of' not in st.session_state: st.session_state['cep_version_of'] = 0
    
    # 🔥 CRIAR TABS: IMPORTAÇÃO AVULSA E GESTÃO DE PEDIDOS FIXOS 🔥
    tab_import, tab_fixos_of = st.tabs(["📋 1. Importação Avulsa (Matriz)", "🔁 2. Gestão de Pedidos Fixos"])
    ...
    with tab_import:
                ...
                if dia_atual != 'DOM':
                    ...
                    if not df_alvo.empty:
                        try:
                            ...
                            # Pular os IDs que já estão no carrinho manual para não haver duplicidade
        ...
        if incluir_fixos_of and not df_fixos_hoje_of.empty:
            if df_cart_of.empty:
                ...
                df_cart_of = pd.concat([df_cart_of, df_fixos_hoje_of], ignore_index=True)
...
# =============================================================================
                    ...
                    with st.spinner("Gerando IDs 700020+ e adicionando ao carrinho..."):
                        try:
                            ...
                            if aba_contador:
                                ...
                                else: aba_contador.update("A1", [["700020"]])
    ...
    with tab_carrinho:
        ...
        if not df_sb.empty:
            ...
            with col_cmd1:
                st.download_button("💾 1. Baixar Arquivo .LOC", data=bytes_loc, file_name=f"LOC_GERAL_{hoje_br.strftime('%d%m%y')}.csv", mime="text/csv", use_container_width=True, on_click=notify_loc)
            with col_cmd2:
...
# =============================================================================
    ...
    with t1:
        if df_raw.empty:
                            ...
                            if str(df_raw.at[idx, 'STATUS']).strip().upper() == 'COLETADO':
                                try:
                                    ...
                                    if mask_nuvem.any():
                                        ...
                                        st.success(f"✅ Pedido {str(df_raw.at[idx, 'PEDIDO'])} VALIDADO!"); time.sleep(1.0); carregar_dados_completos.clear(); st.rerun() 
                                except Exception as e: st.error(f"Erro: {e}")
            ...
            with col_bip_dir:
                ...
                st.markdown("<p style='margin-bottom: 5px; font-weight: bold; color: #0F172A; font-size: 14px;'>⏱️ Últimos Bips:</p>", unsafe_allow_html=True)
                ...
                else: st.markdown("<p style='font-size: 12px; color: #94A3B8;'>Nenhum volume bipado ainda.</p>", unsafe_allow_html=True)
    ...
    with t2:
        if df_raw.empty: st.warning("O banco de dados oficial está vazio.")
        else:
            ...
            if not df_conf.empty:
                ...
                df_conf_show = df_conf[['DATA', 'PEDIDO', 'TOMADOR', 'LABORATORIO', 'CIDADE', 'UF']].fillna("").astype(str)
                        ...
                        if len(tomadores_unicos) > 1: st.error(f"🚨 VIOLAÇÃO DE ROTA: Destinos diferentes selecionados.")
                            ...
                            with st.spinner("Selando romaneio..."):
                                ...
                                try:
                                    ...
                                    aba.clear(); aba.update("A1", [df_nuvem.columns.tolist()] + df_nuvem.fillna("").astype(str).values.tolist())
                                    despachar_para_appsheet([{'PEDIDO': id_romaneio, 'MOTORISTA': motorista_escolhido, 'ENDERECO': "Lote Hub", 'NUMERO': f"{len(p_ids)} V", 'BAIRRO': tomadores_unicos[0], 'CIDADE': sel_lista[0].get('CIDADE', ''), 'CEP': "---", 'LABORATORIO': f"Lote {len(sel_lista)}", 'TOMADOR': tomadores_unicos[0], 'ROMANEIO': id_romaneio}])
                                    pdf_bytes = gerar_pdf_romaneio(id_romaneio, data_despacho, motorista_escolhido, sel_lista)
                                    carregar_dados_completos.clear(); st.success(f"🎉 Lote {id_romaneio} gerado!"); st.download_button(label="📥 BAIXAR PROTOCOLO", data=pdf_bytes, file_name=f"Romaneio_IGO_{id_romaneio}.pdf", mime="application/pdf", type="primary")
                                except Exception as e: st.error(f"Erro: {e}")
            else: st.info("O salão está vazio.")
        ...
        if df_raw.empty: st.warning("O banco de dados oficial está vazio.")
            ...
            if not df_hist.empty:
                ...
                if romaneios_disponiveis:
                    ...
                    rom_sel = col_re_1.selectbox("Selecione o Lote:", ["Selecione..."] + sorted(romaneios_disponiveis, reverse=True))
                    if rom_sel != "Selecione...":
                        ...
                        col_re_2.download_button("📥 REIMPRIMIR", pdf_reprint, file_name=f"Reprint_{rom_sel}.pdf", mime="application/pdf", type="primary", use_container_width=True)
                st.markdown("---")
    ...
    with t4:
        ...
        with st.container(border=True):
            ...
            av_tomador = c1.selectbox("🏢 Hub de Destino:", ["Selecione..."] + CLIENTES_AUTORIZADOS, key="av_hub_man")
...
# =============================================================================
    ...
    data_filtro = st.date_input("📅 Cronograma da Data (Coletas):", value=hoje_br, format="DD/MM/YYYY")
    st.markdown("---")
    ...
    with tab_coletas:
        ...
        with col_esq:
            if df_pendentes.empty: 
                st.success(f"Nenhum volume PENDENTE na data {data_filtro.strftime('%d/%m/%Y')}.")
            else:
                ...
                if agentes_para_enviar:
                    ...
                    st.info(f"🚀 Enviando lote de segurança (5 motoristas). Restam {len(agentes_para_enviar)}.")
                    
                    if st.button(f"🚀 DISPARAR AGORA ({len(lote_atual)} Motoristas)", type="primary", use_container_width=True):
                        pedidos_atualizados = []
                        sucessos = 0
                        progress_bar = st.progress(0)
                        status_text = st.empty()
                        ...
                        for idx_ag, agente in enumerate(lote_atual):
                            ...
                            if telefone:
                                status_text.markdown(f"**Processando:** {nome_amigavel}...")
                                saudacao, fechamento = gerar_saudacao_spintax(nome_amigavel)
                                
                                # TEXTO COMPLETO PARA TODOS (CONTINGÊNCIA) COM SPINTAX
                                sep1 = random.choice(['-------------------------------', '...............................', '=========================', '〰️〰️〰️〰️〰️〰️〰️〰️〰️'])
                                sep2 = random.choice(['---', '...', '===', ' '])
                                bullet = random.choice(['> 🔸', '👉', '📌', '📦', '➖'])
                                lab_lbl = random.choice(['LABORATÓRIO', 'LOCAL', 'PONTO DE COLETA'])
                                
                                msg_parts = [f"{saudacao}rota de 🗓️ {data_filtro.strftime('%d/%m/%Y')}\n", "RESUMO DA ROTA:\n", "CIDADE | QTD", sep1]
                                tot_qtd = 0
                                ...
                                if enviar_whatsapp_zapi(telefone, msg_final):
                                    ...
                                    # CATRACA DE ARQUIVOS (O ANDERSON FICA DE FORA AQUI)
                                    if is_autorizado_pdf:
                                        simular_digitacao_zapi(telefone, 2)
                                        enviar_pdf_zapi(telefone, gerar_pdf_rota_whatsapp(nome_amigavel, data_filtro.strftime('%d/%m/%Y'), df_agente), f"ROTA_{data_filtro.strftime('%d%m')}.pdf")
                                        time.sleep(3)
                                    if is_autorizado_xls:
                                        simular_digitacao_zapi(telefone, 2)
                                        enviar_excel_zapi(telefone, gerar_excel_rota_whatsapp(df_agente), f"ROTA_{data_filtro.strftime('%d%m')}.xlsx")
                                        time.sleep(2)
                                    
                                    sucessos += 1
                                    pedidos_atualizados.extend(df_agente['PEDIDO'].tolist())
                            progress_bar.progress((idx_ag + 1) / len(lote_atual))
                        
                        if pedidos_atualizados:
                            aba = planilha_db.worksheet("Memoria_Sistema")
                            df_nu = pd.DataFrame(aba.get_all_values()[1:], columns=aba.get_all_values()[0])
                            df_nu.loc[df_nu['PEDIDO'].isin(pedidos_atualizados), 'ZAP_ENVIADO'] = f"SIM|{datetime.now(FUSO_BR).strftime('%H:%M')}"
                            aba.clear(); aba.update("A1", [df_nu.columns.tolist()] + df_nu.fillna("").astype(str).values.tolist())
                        st.success(f"✅ Lote processado!"); time.sleep(1.5); st.rerun()

                st.markdown("---")
                for agente in sorted(agentes_com_rota):
                    df_agente = df_pendentes[df_pendentes['AGENTE_RAW'] == agente]
                    nome_ag = dict_nomes.get(str(agente).strip().lower(), str(agente).upper())
                    tel_ag = dict_telefones.get(str(agente).strip().lower(), "")
                    enviado = df_agente['ZAP_ENVIADO'].astype(str).apply(lambda x: str(x).startswith('SIM')).all()
                    ...
                    with st.expander(f"{selo_status} | 👤 {nome_ag}{selo_vip} | Coletas: {len(df_agente)}", expanded=not enviado):
                        ...
                        if st.button("🔄 Reenviar" if enviado else f"📲 Enviar Agora ({nome_ag})", key=f"btn_ind_{agente}", type="secondary" if enviado else "primary"):
                            if not tel_ag: st.error("Sem telefone.")
                            else:
                                is_autorizado_pdf = ag_login in AGENTES_PDF_AUTORIZADOS or login_base in AGENTES_PDF_AUTORIZADOS
                                saudacao, fechamento = gerar_saudacao_spintax(nome_ag)
                                ...
                                if enviar_whatsapp_zapi(tel_ag, msg_final):
                                    ...
                                    if is_autorizado_pdf:
                                        ...
                                        enviar_pdf_zapi(tel_ag, gerar_pdf_rota_whatsapp(nome_ag, data_filtro.strftime('%d/%m/%Y'), df_agente), f"ROTA_{data_filtro.strftime('%d%m')}.pdf")
                                        time.sleep(2)
                                    if is_autorizado_xls:
                                        simular_digitacao_zapi(tel_ag, 2)
                                        enviar_excel_zapi(tel_ag, gerar_excel_rota_whatsapp(df_agente), f"ROTA_{data_filtro.strftime('%d%m')}.xlsx")
                                        
                                    if not enviado:
                                        aba = planilha_db.worksheet("Memoria_Sistema")
                                        df_nu = pd.DataFrame(aba.get_all_values()[1:], columns=aba.get_all_values()[0])
                                        df_nu.loc[df_nu['PEDIDO'].isin(df_agente['PEDIDO'].tolist()), 'ZAP_ENVIADO'] = f"SIM|{datetime.now(FUSO_BR).strftime('%H:%M')}"
                                        aba.clear(); aba.update("A1", [df_nu.columns.tolist()] + df_nu.fillna("").astype(str).values.tolist())
        ...
        if df_entregas.empty: 
            ...
            for rom in sorted(romaneios, reverse=True):
                ...
                with st.expander(f"{'✅ ENVIADO' if lote_enviado else '⏳ AGUARDANDO DISPARO'} | 🔖 Lote: {rom} | 👤 {nome_amigavel} | Volumes: {len(df_rom)}", expanded=not lote_enviado):
                    ...
                    if st.button("🔄 Reenviar Romaneio" if lote_enviado else f"📲 Disparar Romaneio {rom}", key=f"z_rom_{rom}", use_container_width=True, type="secondary" if lote_enviado else "primary"):
                        if telefone:
                            ...
                            if enviar_whatsapp_zapi(telefone, msg_base):
                                ...
                                if is_autorizado_pdf:
                                    simular_digitacao_zapi(telefone, 2)
                                    pdf = gerar_pdf_romaneio(rom, hoje_br, ag, df_rom.to_dict('records'))
                                    enviar_pdf_zapi(telefone, pdf, f"Romaneio_IGO_{rom}.pdf")
                                    time.sleep(2)
                                ...
                                if not lote_enviado:
                                    aba = planilha_db.worksheet("Memoria_Sistema")
                                    df_nu = pd.DataFrame(aba.get_all_values()[1:], columns=aba.get_all_values()[0])
                                    df_nu.loc[df_nu['PEDIDO'].isin(df_rom['PEDIDO'].tolist()), 'ZAP_ENVIADO'] = f"SIM|{datetime.now(FUSO_BR).strftime('%H:%M')}"
                                    aba.clear(); aba.update("A1", [df_nu.columns.tolist()] + df_nu.fillna("").astype(str).values.tolist())
                                st.success("Enviado!"); time.sleep(1.5); st.rerun()
                        else:
            ...
            for cidade in cidades_alvo:
                ...
                try:
                    ...
                    for item in items:
                        ...
                        noticias_radar.append(f"🌍 [RADAR EXTERNO] Trânsito em {cidade.upper()}: {titulo}")
                except: continue
