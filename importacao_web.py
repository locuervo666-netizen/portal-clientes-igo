...
def calc_status_display(row):
    ...
    if res not in ['Entregue', 'Cancelado', 'Frustrada'] and previsao:
        try:
            if datetime.strptime(previsao, "%d/%m/%Y").date() < hoje_br: res = f"{res} (ATRASADO)"
        except: pass
    return res
...
if menu == "📊 Dashboard":
    ...
    if not df_raw.empty:
        ...
        with container_grid:
            ...
            gb.configure_default_column(resizable=True, sortable=True, filter=False, suppressMenu=True, minWidth=120, flex=1)
