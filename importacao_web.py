    ...
    if not df_raw.empty and 'STATUS_DB_ORIGINAL' in df_raw.columns:
        ...
        if not df_to_sync.empty:
            try:
                ...
                dados_nuvem = aba_m.get_all_values()
                if len(dados_nuvem) <= 1:
                    raise ValueError("A aba Memoria_Sistema não possui registros para sincronizar.")
                df_nuvem = pd.DataFrame(
                    dados_nuvem[1:], columns=dados_nuvem[0])
                    ...
                    if c_btn1.button(
                        ...
                        with st.spinner("Processando..."):
                            try:
                                ...
                                dados_nuvem = aba_m.get_all_values()
                                if len(dados_nuvem) <= 1:
                                    raise ValueError("A aba Memoria_Sistema não possui registros para aprovar.")
                                df_nuvem = pd.DataFrame(
                                    dados_nuvem[1:], columns=dados_nuvem[0])
                    ...
                    if c_btn2.button(
                        ...
                        with st.spinner("Recusando..."):
                            try:
                                ...
                                dados_nuvem = aba_m.get_all_values()
                                if len(dados_nuvem) <= 1:
                                    raise ValueError("A aba Memoria_Sistema não possui registros para recusar.")
                                df_nuvem = pd.DataFrame(
                                    dados_nuvem[1:], columns=dados_nuvem[0])
...
# =============================================================================
    ...
    st.markdown("""
        ...
        div.st-key-umove_download_loc div[data-testid="stDownloadButton"] > button:not([kind="primary"]),
        div.st-key-umove_download_loc div[data-testid="stDownloadButton"] > button:not([kind="primary"]):hover {
            background: #155eef !important;
            border-color: #155eef !important;
            color: #ffffff !important;
        }
        div.st-key-umove_download_agd div[data-testid="stDownloadButton"] > button:not([kind="primary"]),
        div.st-key-umove_download_agd div[data-testid="stDownloadButton"] > button:not([kind="primary"]):hover {
            background: #047857 !important;
            border-color: #047857 !important;
            color: #ffffff !important;
        }
        div.st-key-umove_download_excel div[data-testid="stDownloadButton"] > button:not([kind="primary"]),
        div.st-key-umove_download_excel div[data-testid="stDownloadButton"] > button:not([kind="primary"]):hover {
...
# =============================================================================
    ...
    st.markdown(
        ...
        div[data-baseweb="popover"] div[role="listbox"] [role="option"]:hover,
        div[data-baseweb="popover"] div[role="listbox"] li:hover,
        div[data-baseweb="popover"] div[role="listbox"] [aria-selected="true"] {
    ...
    total_carrinho_umove = len(st.session_state.df_sandbox_mem)
    lote_umove = st.session_state.umove_lote_atual_id or "Em preparação"
    status_carrinho_umove = "Pronto para expedição" if total_carrinho_umove else "Aguardando matriz"
    st.markdown(f"""
        <div style="display:flex; justify-content:space-between; align-items:center; gap:24px; margin:0 0 12px; padding:22px 24px; border:1px solid #164e63; border-radius:8px; background:linear-gradient(115deg, #102a43 0%, #164e63 58%, #0f766e 100%); box-shadow:0 12px 26px rgba(15,42,67,0.18);">
            <div style="min-width:0;">
                <div style="margin:0 0 5px; color:#99f6e4; font-size:11px; font-weight:800; letter-spacing:0.8px; text-transform:uppercase;">Centro operacional Umove</div>
                <div style="margin:0; color:#ffffff; font-size:26px; font-weight:800; line-height:1.15;">Importação e Central de Envios</div>
                <div style="max-width:650px; margin:7px 0 0; color:#d9f7f2; font-size:13px; line-height:1.45;">Prepare os pedidos, confira os dados e envie as rotas para os motoristas.</div>
            </div>
            <div style="flex:0 0 auto; min-width:170px; padding:11px 14px; border:1px solid rgba(255,255,255,0.30); border-radius:8px; background:rgba(7,28,45,0.26); text-align:right;">
                <div style="color:#b8f3ea; font-size:10px; font-weight:800; letter-spacing:0.7px; text-transform:uppercase;">Lote atual</div>
                <div style="color:#ffffff; font-size:25px; font-weight:800; line-height:1.1;">{total_carrinho_umove} pedido(s)</div>
                <div style="color:#d9f7f2; font-size:11px; margin-top:3px;">{lote_umove} · {status_carrinho_umove}</div>
            </div>
        </div>
    """, unsafe_allow_html=True)
    ...
    tab_matriz, tab_carrinho, tab_envios, tab_backup, tab_fixos = st.tabs(
        ["📋 1. Importar", "🛒 2. Carrinho", "🚀 3. Enviar", "🛟 4. Recuperar", "🔁 5. Pedidos Fixos"])
    ...
    with tab_matriz:
        ...
        with st.container(border=True):
            ...
            with c1_sb:
                st.markdown("<div style='margin:0 0 4px; color:#0f172a; font-size:14px; font-weight:800;'>🏢 TOMADOR DESTA CARGA <span style='color:#dc2626;'>*</span></div>", unsafe_allow_html=True)
                tom_sandbox = st.selectbox(
                    "Tomador desta carga",
                    ["Selecione..."] + CLIENTES_AUTORIZADOS,
                    key="tom_sb",
                    label_visibility="collapsed",
                    help="Todos os pedidos colados nesta matriz serão vinculados ao tomador selecionado.",
                )
            with c2_sb:
                dt_sandbox = st.date_input("📅 Data da Rota *", format="DD/MM/YYYY", value=hoje_br, key="dt_sb")

            if tom_sandbox == "Selecione...":
                st.markdown("<div style='margin-top:10px; padding:9px 12px; border-left:4px solid #f59e0b; background:#fffbeb; color:#92400e; font-size:13px; font-weight:700;'>Selecione o tomador antes de colar os pedidos.</div>", unsafe_allow_html=True)
            else:
                st.markdown(f"<div style='margin-top:10px; padding:9px 12px; border-left:4px solid #0ea5e9; background:#eff6ff; color:#075985; font-size:13px; font-weight:800;'>Cliente selecionado para esta matriz: {escape(str(tom_sandbox))}</div>", unsafe_allow_html=True)

        # Passo 2: Inserção
        st.markdown("#### 2️⃣ Cole os Dados")
        with st.container(border=True):
            ...
            @st.dialog("Confirmar inclusão no carrinho")
            def confirmar_importacao_umove(qtd_pedidos, tomador, assinatura_previa):
                st.markdown(f"### Adicionar {qtd_pedidos} pedido(s) ao carrinho?")
                st.markdown(f"Os pedidos serão incluídos para o tomador **{tomador}**.")
                st.warning("Confira o tomador antes de continuar. Esta ação adiciona os pedidos ao carrinho de expedição.")
                col_confirmar, col_cancelar = st.columns(2)
                if col_confirmar.button("Confirmar e adicionar", type="primary", use_container_width=True):
                    st.session_state.umove_adicao_confirmada = {
                        "tomador": tomador,
                        "assinatura": assinatura_previa,
                    }
                    st.session_state.pop("umove_adicao_pendente", None)
                    st.rerun()
            ...
            solicitar_processamento = st.button("🔍 PROCESSAR E VALIDAR MATRIZ", type="primary", use_container_width=True)
            if solicitar_processamento:
                if not txt_sb or tom_sandbox == "Selecione...":
                    st.error("⚠️ Selecione o tomador e cole os dados da matriz antes de continuar.")
            if solicitar_processamento and txt_sb and tom_sandbox != "Selecione...":
        ...
        if not st.session_state.df_preview_sb.empty:
            ...
            if not df_err.empty:
                ...
                st.write("")
                tomadores_previa = set(df_ok['TOMADOR'].astype(str).str.strip()) if 'TOMADOR' in df_ok.columns else set()
                previa_confirmada = (
                    tom_sandbox != "Selecione..."
                    and tomadores_previa == {str(tom_sandbox).strip()}
                )
                if not previa_confirmada:
                    st.warning("A prévia não corresponde ao tomador selecionado. Selecione o cliente correto e processe a matriz novamente antes de adicionar ao carrinho.")
                solicitar_adicao = st.button(
                    "➕ ADICIONAR TUDO AO CARRINHO DE EXPEDIÇÃO",
                    type="primary",
                    use_container_width=True,
                    key="add_carrinho_sb",
                    disabled=not previa_confirmada,
                )
                assinatura_previa = hashlib.sha256(
                    df_ok.to_csv(index=False).encode("utf-8")
                ).hexdigest()
                if solicitar_adicao:
                    st.session_state.umove_adicao_pendente = {
                        "tomador": tom_sandbox,
                        "assinatura": assinatura_previa,
                        "qtd_pedidos": len(df_ok),
                    }

                pendencia_adicao = st.session_state.get("umove_adicao_pendente")
                if pendencia_adicao:
                    if (
                        pendencia_adicao.get("tomador") != tom_sandbox
                        or pendencia_adicao.get("assinatura") != assinatura_previa
                    ):
                        st.session_state.pop("umove_adicao_pendente", None)
                    else:
                        confirmar_importacao_umove(
                            pendencia_adicao["qtd_pedidos"],
                            pendencia_adicao["tomador"],
                            pendencia_adicao["assinatura"],
                        )

                adicao_confirmada = st.session_state.get("umove_adicao_confirmada")
                adicionar_ao_carrinho = (
                    adicao_confirmada is not None
                    and adicao_confirmada.get("tomador") == tom_sandbox
                    and adicao_confirmada.get("assinatura") == assinatura_previa
                )
                if adicionar_ao_carrinho:
                    st.session_state.pop("umove_adicao_confirmada", None)
                    with st.spinner("Registrando identificadores de controle (IDs) blindados..."):
                        try:
                            ...
                            st.session_state.df_preview_sb = pd.DataFrame()
                            st.session_state.ui_toast = {'msg': f"{len(df_ok)} pedido(s) adicionado(s) ao carrinho com sucesso.", 'icon': "✅"}
                            st.rerun()
                        except Exception as e:
    ...
    with tab_fixos:
        ...
        with st.form("form_novo_fixo", clear_on_submit=True):
            col_f1, col_f2 = st.columns(2)
            f_tomador = col_f1.selectbox("🏢 **TOMADOR DO PEDIDO FIXO** *", ["Selecione..."] + CLIENTES_AUTORIZADOS)
            f_lab = col_f2.text_input("Ponto de Coleta / Laboratório *")
    ...
    with tab_carrinho:
        ...
        try:
            ...
            if len(dados_fixos) > 1:
                ...
                mapa_dias = {0: 'SEG', 1: 'TER', 2: 'QUA', 3: 'QUI', 4: 'SEX', 5: 'SAB', 6: 'DOM'}
                dia_agendamento = mapa_dias[data_agendamento_fixos.weekday()]

                if dia_agendamento != 'DOM':
                    df_alvo = df_regras_temp[(df_regras_temp[dia_agendamento] == "SIM") & (df_regras_temp['STATUS'] == "ATIVO")].copy()
                    if not df_alvo.empty:
                        ...
                        for _, regra in df_alvo.iterrows():
                            novo_pedido = {
                                'DATA': data_agendamento_fixos.strftime("%d/%m/%Y"),
        ...
        if not st.session_state.df_sandbox_mem.empty and 'OBSERVACOES' in st.session_state.df_sandbox_mem.columns:
            mascara_fixos = st.session_state.df_sandbox_mem['OBSERVACOES'].astype(str).str.contains(r'\[FIXO\]', regex=True, na=False)
            mascara_data_fixa = st.session_state.df_sandbox_mem['DATA'].astype(str).eq(data_agendamento_fixos.strftime("%d/%m/%Y")) if 'DATA' in st.session_state.df_sandbox_mem.columns else False
            tem_fixos_na_memoria = (mascara_fixos & mascara_data_fixa).any()
        ...
        if mostrar_painel_fixos:
            ...
            dia_msg = f" para <b>{data_agendamento_fixos.strftime('%d/%m/%Y')}</b>"
            ...
            if incluir_fixos != tem_fixos_na_memoria:
                if incluir_fixos:
                    ...
                    with st.spinner("Desfazendo injeção de pedidos fixos..."):
                        ...
                        mask_data_fixa = st.session_state.df_sandbox_mem['DATA'].astype(str).eq(data_agendamento_fixos.strftime("%d/%m/%Y"))
                        st.session_state.df_sandbox_mem = st.session_state.df_sandbox_mem[~(mask_fixos & mask_data_fixa)]
        ...
        if not df_sb.empty:
            ...
            with col_env3:
                st.markdown("<div class='of-card'><div class='of-card-title'>Etapa 3</div><div class='of-card-subtitle'><b>Baixar arquivos</b><br>Baixe a planilha ou gere os arquivos para integração.</div></div>", unsafe_allow_html=True)
            ...
            with col_cmd1:
                st.download_button("💾 Baixar Arquivo de Locais (.LOC)", data=bytes_loc, file_name=f"LOC_GERAL_{hoje_br.strftime('%d%m%y')}.csv", mime="text/csv", use_container_width=True, on_click=lambda: st.toast("Baixando arquivo LOC...", icon="📥"), key="umove_download_loc")
            with col_cmd2:
                st.download_button("💾 Baixar Arquivo de Agendas (.AGD)", data=bytes_agd, file_name=f"AGD_GERAL_{hoje_br.strftime('%d%m%y')}.csv", mime="text/csv", use_container_width=True, on_click=lambda: st.toast("Baixando arquivo AGD...", icon="📥"), key="umove_download_agd")
            with col_cmd3:
                st.download_button(
    ...
    with tab_envios:
        ...
        def salvar_backup_completo_umove(df_bkp, id_ev, planilha):
            ...
            try:
                try:
                    aba_bkp = planilha.worksheet("Backup_Umove_Rotas")
                except Exception:
                    aba_bkp = planilha.add_worksheet("Backup_Umove_Rotas", 100, 20)
                    aba_bkp.update("A1", [["ID_EVENTO", "DATA_DISPARO"] + df_bkp.columns.tolist()])

                df_para_backup = df_bkp.copy()
                df_para_backup.insert(0, "DATA_DISPARO", datetime.now(FUSO_BR).strftime('%d/%m/%Y %H:%M:%S'))
                df_para_backup.insert(0, "ID_EVENTO", id_ev)
                aba_bkp.append_rows(
                    df_para_backup.fillna("").astype(str).values.tolist(),
        ...
        if df_fonte_envio.empty and st.session_state.umove_step not in ['PROCESSING', 'COMPLETED']:
            ...
            if st.session_state.umove_step in ['IDLE', 'CONFIRMING']:
                ...
                if not backup_ok:
                    sync_pendencias.append("Falha ao salvar backup do lote na planilha Umove")
...
# =============================================================================
    ...
    def gerar_pdf_triagem_manual(id_lote, data_str, tomador, lista_itens):
        ...
        pdf.cell(0, 4, f"Data da Triagem: {dt_s} | Cliente: {tomador} | Emitido em: {data_emissao}", ln=True, align="C")
        pdf.ln(3)
        pdf.line(10, pdf.get_y(), 200, pdf.get_y())
        pdf.ln(3)
        ...
        if protocolo_importado:
            larguras_tabela = [8, 18, 54, 24, 32, 48, 12]
        else:
        ...
        if protocolo_importado:
            ...
            pdf.cell(18, 6, "ID ORIGEM", 1, 0, "C", True)
            pdf.cell(54, 6, "PC ORIGEM", 1, 0, "C", True)
            pdf.cell(24, 6, "DATA COLETA", 1, 0, "C", True)
            pdf.cell(32, 6, "DATA CONFERENCIA", 1, 0, "C", True)
            pdf.cell(48, 6, "CIDADE", 1, 0, "C", True)
            pdf.cell(12, 6, "UF", 1, 1, "C", True)
        ...
        pdf.set_text_color(51, 65, 85)
        pdf.set_font("Arial", "", 5 if protocolo_importado else (5 if tem_campos_opcionais else 8))
        ...
        for idx, item in enumerate(lista_itens, 1):
            ...
            data_coleta = str(item.get('DATA_COLETA', item.get('DATA', ''))).strip()
            data_conferencia = str(item.get('DATA_CONFERENCIA', '')).strip()
            dt_bip = str(item.get('DATA', ''))
            hr_bip = str(item.get('HORA', ''))

            pdf.set_x(inicio_tabela)
            if protocolo_importado:
                pdf.cell(8, 6, str(idx), 1, 0, "C", True)
                pdf.cell(18, 6, cortar_texto(pedido, 9), 1, 0, "C", True)
                pdf.cell(54, 6, cortar_texto(cliente, 28), 1, 0, "C", True)
                pdf.cell(24, 6, cortar_texto(data_coleta, 12), 1, 0, "C", True)
                pdf.cell(32, 6, cortar_texto(data_conferencia, 17), 1, 0, "C", True)
                pdf.cell(48, 6, cortar_texto(cidade, 24), 1, 0, "C", True)
                pdf.cell(12, 6, cortar_texto(uf, 3), 1, 1, "C", True)
            elif tem_campos_opcionais:
        ...
        pdf.line(120, pdf.get_y(), 190, pdf.get_y())
        pdf.set_font("Arial", "B", 7)
        pdf.cell(95, 4, "ASSINATURA CONFERENTE", 0, 0, "C")
        pdf.cell(95, 4, "ASSINATURA DO RECEBEDOR", 0, 1, "C")
    ...
    def converter_linhas_importadas_triagem(df_importado, coluna_pedido, coluna_cliente, coluna_data_coleta, coluna_data_conferencia, coluna_cidade, coluna_uf, coluna_envelope, tomador, data_triagem):
        ...
        for _, linha in df_importado.iterrows():
            ...
            data_coleta = str(linha.get(coluna_data_coleta, '')).strip() if coluna_data_coleta else ''
            data_conferencia = str(linha.get(coluna_data_conferencia, '')).strip() if coluna_data_conferencia else ''
            ...
            itens.append({
                ...
                'DATA_COLETA': data_coleta or data_triagem.strftime('%d/%m/%Y'),
                    ...
                    if df_sel_fila.empty:
                        ...
                        with st.spinner("Conferindo lote em massa..."):
                            ...
                            try:
                                ...
                                dados_nuvem = aba.get_all_values()
                                if len(dados_nuvem) <= 1:
                                    raise ValueError("A aba Memoria_Sistema não possui registros para processar.")
                                cabecalho_original = [str(coluna).strip().upper() for coluna in dados_nuvem[0]]
                                if 'PEDIDO' not in cabecalho_original or 'STATUS' not in cabecalho_original:
                                    raise ValueError("As colunas PEDIDO e STATUS são obrigatórias para a validação.")
                                cabecalho = cabecalho_original + [
                                    coluna for coluna in COLUNAS_TRANSBORDO if coluna not in cabecalho_original
                                ]
                                df_nuvem = pd.DataFrame(dados_nuvem[1:], columns=cabecalho_original)
                                ...
                                pedidos_selecionados = set(p_ids)
                                linhas_selecionadas = df_nuvem[
                                    df_nuvem['PEDIDO'].astype(str).isin(pedidos_selecionados)
                                ]
                                if linhas_selecionadas.empty:
                                    raise ValueError("Nenhum pedido selecionado foi localizado na planilha.")

                                indices_colunas = {coluna: cabecalho.index(coluna) + 1 for coluna in cabecalho}
                                updates = [
                                    {
                                        'range': gspread.utils.rowcol_to_a1(1, indices_colunas[coluna]),
                                        'values': [[coluna]],
                                    }
                                    for coluna in COLUNAS_TRANSBORDO if coluna not in cabecalho_original
                                ]
                                for indice_df, linha in linhas_selecionadas.iterrows():
                                    ...
                                    if fluxo['transbordo']:
                                        total_transbordo += 1
                                        volumes_lote_transbordo.append(linha.to_dict())
                                        valores_atualizados = {
                                            'STATUS': 'EM TRANSBORDO',
                                            'ETAPA_LOGISTICA': fluxo['etapa'],
                                            'HUB_ORIGEM': fluxo['origem'],
                                            'HUB_DESTINO': fluxo['destino'],
                                            'DATA_TRANSBORDO': agora_transbordo.strftime('%d/%m/%Y %H:%M:%S'),
                                            'LOTE_TRANSBORDO': id_lote_transbordo,
                                            'QR_TRANSBORDO': id_lote_transbordo,
                                        }
                                    else:
                                    ...
                                    for coluna, valor in valores_atualizados.items():
                                        updates.append({
                                            'range': gspread.utils.rowcol_to_a1(numero_linha, indices_colunas[coluna]),
                                            'values': [[valor]],
                                        })
                                aba.batch_update(updates)
                        ...
                        if st.button(f"↩️ Reverter Selecionados para Triagem ({qtd_sel_rev} vol.) — Desfaz Bipagem", use_container_width=True, type="secondary"):
                            ...
                            with st.spinner("Revertendo bipagem e devolvendo para Aba 1..."):
                                try:
                                    ...
                                    dados_nuvem = aba.get_all_values()
                                    if len(dados_nuvem) <= 1:
                                        raise ValueError("A aba Memoria_Sistema não possui registros para reverter.")
                                    cabecalho = [str(coluna).strip().upper() for coluna in dados_nuvem[0]]
                                    if 'PEDIDO' not in cabecalho or 'STATUS' not in cabecalho:
                                        raise ValueError("As colunas PEDIDO e STATUS são obrigatórias para a reversão.")

                                    colunas_para_limpar = ['DATA_BIP', 'HORA_BIP', 'ROMANEIO']
                                    colunas_indices = {coluna: cabecalho.index(coluna) + 1 for coluna in ['STATUS'] + colunas_para_limpar if coluna in cabecalho}
                                    pedidos_reverter = set(p_ids_reverter)
                                    updates = []
                                    indice_pedido = cabecalho.index('PEDIDO')
                                    for numero_linha, linha in enumerate(dados_nuvem[1:], start=2):
                                        pedido = str(linha[indice_pedido]).strip() if indice_pedido < len(linha) else ''
                                        if pedido not in pedidos_reverter:
                                            continue
                                        updates.append({
                                            'range': gspread.utils.rowcol_to_a1(numero_linha, colunas_indices['STATUS']),
                                            'values': [['COLETADO']],
                                        })
                                        for coluna in colunas_para_limpar:
                                            if coluna in colunas_indices:
                                                updates.append({
                                                    'range': gspread.utils.rowcol_to_a1(numero_linha, colunas_indices[coluna]),
                                                    'values': [['']],
                                                })
                                    if not updates:
                                        raise ValueError("Nenhum pedido selecionado foi localizado para reversão.")
                                    aba.batch_update(updates)
                            ...
                            if em_transbordo:
                                ...
                                with st.spinner("Selando romaneio e gerando documentação..."):
                                    ...
                                    try:
                                        ...
                                        if len(dados_nuvem) > 1:
                                            cabecalho = [str(coluna).strip().upper() for coluna in dados_nuvem[0]]
                                            colunas_obrigatorias = ['PEDIDO', 'STATUS', 'ROMANEIO', 'AGENTE_RAW']
                                            colunas_ausentes = [coluna for coluna in colunas_obrigatorias if coluna not in cabecalho]
                                            if colunas_ausentes:
                                                raise ValueError(f"Colunas obrigatórias ausentes: {', '.join(colunas_ausentes)}")
                                            indices_colunas = {coluna: cabecalho.index(coluna) + 1 for coluna in colunas_obrigatorias}
                                            pedidos_selecionados = set(p_ids_sel)
                                            indice_pedido = cabecalho.index('PEDIDO')
                                            updates = []
                                            for numero_linha, linha in enumerate(dados_nuvem[1:], start=2):
                                                pedido = str(linha[indice_pedido]).strip() if indice_pedido < len(linha) else ''
                                                if pedido not in pedidos_selecionados:
                                                    continue
                                                for coluna, valor in {
                                                    'STATUS': 'EM ROTA DE ENTREGA',
                                                    'ROMANEIO': id_romaneio,
                                                    'AGENTE_RAW': motorista_escolhido,
                                                }.items():
                                                    updates.append({
                                                        'range': gspread.utils.rowcol_to_a1(numero_linha, indices_colunas[coluna]),
                                                        'values': [[valor]],
            ...
            if st.button("✅ Receber e Criar Romaneio de Entrega Final", type="primary", use_container_width=True):
                if not pedidos_recebidos or motorista_final == "Selecione...":
                    ...
                    try:
                        ...
                        dados_nuvem = aba.get_all_values()
                        if len(dados_nuvem) <= 1:
                            raise ValueError("A aba Memoria_Sistema não possui registros para receber.")
                        cabecalho_original = [str(coluna).strip().upper() for coluna in dados_nuvem[0]]
                        if 'PEDIDO' not in cabecalho_original:
                            raise ValueError("A coluna PEDIDO é obrigatória para o recebimento.")
                        cabecalho = cabecalho_original + [
                            coluna for coluna in COLUNAS_TRANSBORDO if coluna not in cabecalho_original
                        ]
                        df_nuvem = pd.DataFrame(dados_nuvem[1:], columns=cabecalho_original)
                        ...
                        data_recebimento = datetime.now(FUSO_BR).strftime('%d/%m/%Y %H:%M:%S')
                        for indice_df in df_nuvem.index[mask_recebimento]:
                            numero_linha = indice_df + 2
                            for coluna, valor in {
                                'STATUS': 'EM ROTA DE ENTREGA',
                                'ROMANEIO': id_romaneio,
                                'AGENTE_RAW': motorista_final,
                                'ETAPA_LOGISTICA': 'RECEBIDO_HUB_DESTINO',
                                'DATA_RECEBIMENTO_HUB': data_recebimento,
                            }.items():
                                updates.append({
                                    'range': gspread.utils.rowcol_to_a1(numero_linha, indices_colunas[coluna]),
                                    'values': [[valor]],
                ...
                if arquivo_triagem is not None:
                    try:
                        ...
                        if df_importado_triagem.empty:
                            ...
                            coluna_data_coleta_auto = encontrar_coluna_importacao(
                                colunas_arquivo,
                                ['Data Coleta', 'DATA_COLETA', 'Data da Coleta', 'DATA DA COLETA', 'Data']
                            )
                            coluna_data_conferencia_auto = encontrar_coluna_importacao(
                                colunas_arquivo,
                                ['Data/hora da aprovação', 'Data Hora da Aprovacao', 'Data Conferencia', 'DATA_CONFERENCIA', 'Data da Conferencia']
                            )
                            ...
                            opcoes_data_coleta = ['Usar data da triagem'] + colunas_arquivo
                            data_coleta_auto_opcao = coluna_data_coleta_auto if coluna_data_coleta_auto in colunas_arquivo else 'Usar data da triagem'
                            coluna_data_coleta_imp = ci3.selectbox(
                                "Coluna Data Coleta:",
                                opcoes_data_coleta,
                                index=opcoes_data_coleta.index(data_coleta_auto_opcao),
                                key='coluna_data_coleta_triagem_importada'
                            )
                            opcoes_data_conferencia = ['Não informar'] + colunas_arquivo
                            data_conferencia_auto_opcao = coluna_data_conferencia_auto if coluna_data_conferencia_auto in colunas_arquivo else 'Não informar'
                            coluna_data_conferencia_imp = ci4.selectbox(
                                "Coluna Data Conferência:",
                                opcoes_data_conferencia,
                                index=opcoes_data_conferencia.index(data_conferencia_auto_opcao),
                                key='coluna_data_conferencia_triagem_importada'
                            )
                            coluna_cidade_imp = ci5.selectbox(
                                "Coluna Cidade:",
                                colunas_arquivo,
                            ...
                            coluna_envelope_imp = ci7.selectbox(
                                "Coluna do Envelope:",
                                opcoes_envelope,
                            ...
                            if coluna_data_coleta_imp != 'Usar data da triagem':
                                colunas_preview.insert(2, coluna_data_coleta_imp)
                            if coluna_data_conferencia_imp != 'Não informar':
                                colunas_preview.insert(3, coluna_data_conferencia_imp)
                            ...
                            if st.button("➕ Importar registros para a cesta", type="primary", use_container_width=True, key='importar_triagem_manual'):
                                if av_tomador == "Selecione...":
                                    ...
                                    coluna_data_coleta_final = None if coluna_data_coleta_imp == 'Usar data da triagem' else coluna_data_coleta_imp
                                    coluna_data_conferencia_final = None if coluna_data_conferencia_imp == 'Não informar' else coluna_data_conferencia_imp
                                    itens_importados = converter_linhas_importadas_triagem(
                                        df_importado_triagem,
                                        coluna_pedido_imp,
                                        coluna_cliente_imp,
                                        coluna_data_coleta_final,
                                        coluna_data_conferencia_final,
                                        coluna_cidade_imp,
                                        coluna_uf_imp,
