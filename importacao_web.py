...
def carregar_dados_completos(_planilha):
    ...
    try:
        ...
        if len(dados_m) > 1:
            ...
            return df
    except Exception as e: st.error(f"Erro ao carregar banco: {e}")
    return pd.DataFrame()
...
# =============================================================================
# 🚀 MÓDULO 1: DASHBOARD (NATIVO STREAMLIT - SEM SUMIR)
# =============================================================================
if menu == "📊 Dashboard":
    ...
    if not df_raw.empty:
        ...
        st.markdown(f"<p style='color:#059669; font-weight:600; font-size:12px; margin-bottom: 5px;'>🟢 Sincronizado: {datetime.now(FUSO_BR).strftime('%H:%M:%S')} | Marque a caixinha na tabela para acessar os Botões e a Foto.</p>", unsafe_allow_html=True)
        ...
        # TABELA NATIVA DO STREAMLIT: NÃO SOME NUNCA.
        ...
        # 🔥 RENDERIZA OS BOTÕES DENTRO DO CONTAINER DO TOPO, LENDO A SELEÇÃO ATUAL
        ...
        # 🔥 A FOTO SÓ APARECE EMBAIXO SE TIVER SELEÇÃO
        if tem_sel:
            st.markdown("---")
            ...
            if not fotos_validas:
                ...
                cols_fotos = st.columns(len(fotos_validas) if len(fotos_validas) <= 4 else 4)
                ...
                if m_tomador == "Selecione..." or not m_cid or not m_lab or not m_rua or not m_bai: 
                    ...
                    with st.spinner("Padronizando textos e salvando na nuvem..."):
                        ...
                        try:
                            ...
                            st.success(f"🎉 Pedido {m_pedido} criado com sucesso!"); carregar_dados_completos.clear()
                    ...
                    if not tem_selecao: st.warning("⚠️ Marque os pedidos na tabela acima primeiro!")
                        ...
                        with st.spinner("Atualizando pedidos selecionados em lote..."):
                            ...
                            try:
                                ...
                                aba.clear(); aba.update("A1", [df_nuvem.columns.tolist()] + df_nuvem.fillna("").astype(str).values.tolist())
                                st.success(f"🎉 {len(p_ids)} pedidos enviados para o Despacho!"); carregar_dados_completos.clear(); st.rerun()
                            except Exception as e: st.error(f"Erro: {e}")
            else:
        ...
        with t2:
            ...
            if not df_conf.empty:
                ...
                if c_btn.button("🚚 Despachar e Gerar PDF", type="primary", use_container_width=True):
                    if not tem_sel_pdf or motorista_escolhido == "Selecione...": st.warning("⚠️ Exigência de Rota: Marque os pacotes na tabela acima e informe o responsável.")
                    else:
                        with st.spinner("Gerando Romaneio PDF (Selo IGO)..."):
                            ...
                            try:
                                ...
                                pdf.cell(10, 5, "ITEM", 1, 0, "C", True); pdf.cell(25, 5, "PEDIDO", 1, 0, "C", True); pdf.cell(30, 5, "ID CLIENTE", 1, 0, "C", True)
                                pdf.cell(80, 5, "PONTO DE COLETA / LABORATÓRIO", 1, 0, "C", True); pdf.cell(35, 5, "CIDADE", 1, 0, "C", True); pdf.cell(10, 5, "UF", 1, 1, "C", True)
                                ...
                                for idx, item in enumerate(sel_lista, 1):
                                    ...
                                    pdf.cell(10, 5, str(idx), 1, 0, "C", True); pdf.cell(25, 5, str(item.get('PEDIDO','')), 1, 0, "C", True); pdf.cell(30, 5, qr_val, 1, 0, "C", True)
                                    pdf.cell(80, 5, str(item.get('LABORATORIO',''))[:48], 1, 0, "L", True); pdf.cell(35, 5, str(item.get('CIDADE',''))[:22], 1, 0, "L", True); pdf.cell(10, 5, str(item.get('UF','')), 1, 1, "C", True)
