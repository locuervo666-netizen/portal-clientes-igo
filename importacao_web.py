    ...
    with st.form("form_login"):
        st.markdown("""
            <div style="text-align: center; margin-bottom: 25px;">
                ...
                <div style="color: #0F172A; font-weight: 800; font-size: 20px; margin-top: 15px;">PORTAL CORPORATIVO</div>
                <div style="color: #64748B; font-size: 13px;">Autenticação de Operadores</div>
            </div>
        """, unsafe_allow_html=True)
            ...
            try:
                ...
                if len(dados_app) > 1:
                    ...
                    def get_true_status(row):
                        s_db, s_app, rom_id = str(row.get('STATUS', '')).strip().upper(), str(row.get('APP_STATUS', '')).strip().upper(), str(row.get('ROMANEIO', '')).strip()
...
# =============================================================================
# 🚀 MÓDULO 1: DASHBOARD (NOVA ARQUITETURA BLINDADA)
# =============================================================================
if menu == "📊 Dashboard":
    ...
    if not df_raw.empty:
        ...
        # 🔥 ROTA DE FUGA DA TELA BRANCA: st.dataframe Pura (inquebrável) com Links
        st.dataframe(
            df_grid,
            hide_index=True,
            ...
            height=450,
            column_config={
                "FOTO_URL": st.column_config.LinkColumn("📸 Link da Foto", display_text="Abrir Foto")
            }
        )

        # 🔥 NOVO MOTOR DE SELEÇÃO DESACOPLADO DA TABELA (100% à prova de crash)
        st.markdown("---")
        st.markdown("### ⚡ Ações Rápidas (Baixa, Clone e Troca)")
        p_ids = st.multiselect("🎯 Selecione os códigos dos pedidos acima para aplicar uma ação:", df_grid['PEDIDO'].tolist())
        tem_sel = len(p_ids) > 0
    ...
    if not df_raw.empty:
        ...
        with t1:
            ...
            if not df_fila.empty:
                ...
                p_ids = st.multiselect("🎯 Selecione os pedidos para Enviar:", df_fila['PEDIDO'].tolist())
        ...
        with t2:
            ...
            if not df_conf.empty:
                ...
                p_ids_rom = st.multiselect("🎯 Selecione os pedidos para Despachar:", df_conf['PEDIDO'].tolist())
